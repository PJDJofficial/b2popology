...
export class PropertyGlue extends Property {

  clone() {
    return new PropertyGlue(this.key, structuredClone(this.val));
  }
  ...
  toHTML() {
    ...
    title.textContent = 'Applies glued status to bloons';
      ...
      case 'merge':
        Object.keys(buff.value).forEach((key) => {
          if (this.val[key] == null) {
            console.warn('Cannot merge with key that doesn\'t exist');
            return;
          };
          this.val[key].push(buff.value[key]);
        });
        break;
      case 'add':
        Object.keys(buff.value).forEach((key) => {
